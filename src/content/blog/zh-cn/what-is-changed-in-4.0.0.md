...
title: 'v4.0.0 的新变化'
description: 'AstrBot v4.0.0 带来多配置文件、配置热重载、图片转述、MCP 同步等更新。'
---
...
在 v4 版本中，我们引入了配置文件热重载功能。用户可以在不重启服务的情况下，实时更新配置文件的内容。这一功能极大地方便了用户的配置管理，提高了系统的灵活性。

> 注意：在修改配置文件后，您仍需手动点击右下角的保存按钮，否则更改将不会生效。

### 图片转述

我们注意到部分 LLM 并没有多模态能力，如 `deepseek-chat`，而这类模型仍然在被很多用户使用。因此我们引入了图片转述功能，允许用户设置一个专门用于图片转述的模型。图片将会由图片转述模型转述为文本再移交给聊天模型。
...
### 同时启动多个相同的消息平台适配器会导致上下文混乱

问题原因：v3 版本用于标识会话的 umo 中 platform_id 字段被错误的设置为了消息平台的类型字符串，导致多个相同类型的消息平台实例在相同的会话中共享了 umo，即共享同一个聊天上下文，导致混乱。

解决方案：在 v4 版本中，我们对 platform_id 的设置进行了修正，确保每个消息平台实例都有独立的上下文。
